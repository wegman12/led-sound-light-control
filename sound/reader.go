...
type reader struct {
	...
	usePRU        bool // If true, use PRU reader; if false, use IIO with decimation
	useIIOFast    bool // If true, use high-speed IIO with decimation
}
...
func (r *reader) readFromIIOLegacy(ctx context.Context) {
	fmt.Println("IIO legacy reader not implemented")
	fmt.Println("Please use IIO fast mode by setting reader.useIIOFast = true")
